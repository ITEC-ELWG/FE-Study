...
function ifCanMove(btn){  
         ...
         var check=String(dim_num*dim_num);
         box_btn=$(".btn_num");
         if($(box_btn[btn_index-dim_num]).attr('value')==check||$(box_btn[btn_index-1]).attr('value')==check||$(box_btn[btn_index+1]).attr('value')==check||$(box_btn[btn_index+dim_num]).attr('value')==check){
                move_flag=true;
         }
